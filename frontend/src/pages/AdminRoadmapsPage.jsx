    ...
    <div className="space-y-6" data-testid="admin-roadmaps-page">
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-zinc-500 mt-1">
            {pipelineType === 'sales_pipeline'
              ? (lang === 'uk' ? 'Воронка продажів по компанії' : lang === 'ru' ? 'Воронка продаж по компании' : lang === 'bg' ? 'Фуния на продажбите' : 'Sales pipeline across the company.')
        ...
        <div className="inline-flex bg-zinc-100 rounded-xl p-0.5" data-testid="admin-roadmaps-type-switcher">
          ...
          >
            {lang === 'uk' ? 'Продажі' : lang === 'ru' ? 'Продажи' : lang === 'bg' ? 'Продажби' : 'Sales'}
          </button>
          <button
          ...
          >
            {lang === 'uk' ? 'Авто' : lang === 'ru' ? 'Авто' : lang === 'bg' ? 'Автомобил' : 'Vehicle'}
          </button>
        </div>
      </header>
